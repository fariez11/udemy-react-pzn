...
function Result() {
    ...
    return (
        <>
            <h3>Online Check</h3>
            <div className={'card text-center ' + (isOnline ? 'bg-success-subtle' : 'bg-danger-subtle')}>
                <h5 className={'my-5 ' + (isOnline ? 'text-success' : 'text-danger')}><i className={isOnline ? 'bi bi-check-circle' : 'bi bi-exclamation-circle'}></i> {isOnline ? 'Online' : 'Offline'}</h5>
            </div>
